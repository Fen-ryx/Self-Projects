    ...
    while True:
        ...
        if len(cur_batch) == batch_size:
            ...
            for li in cur_batch:
                ...
                # [1, 1, 1, 0, 0, 0]
                example_mask = [1 for i in range(len(tensor))] + [0 for j in range(max_length - len(tensor))]
...
def GRULM(vocab_size=256, d_model=512, n_layers=2, mode='train'):
    ...
    ### START CODE HERE ###
    model = tl.Serial( 
      tl.ShiftRight(1, mode), # Stack the ShiftRight layer
...
def train_model(model, data_generator, lines, eval_lines, batch_size=32, max_length=64, n_steps=1, output_dir='model/'): 
    ...
    ### START CODE HERE ###
    bare_train_generator = data_generator(batch_size, max_length, lines, line_to_tensor=line_to_tensor)
    infinite_train_generator = itertools.cycle(data_generator(batch_size, max_length, lines, line_to_tensor=line_to_tensor))
    
    bare_eval_generator = data_generator(batch_size, max_length, eval_lines, line_to_tensor=line_to_tensor)
    infinite_eval_generator = itertools.cycle(data_generator(batch_size, max_length, eval_lines, line_to_tensor=line_to_tensor))

...
# Now import a function that processes a tweet (we've provided this in the utils.py file).
...
def data_generator(data_pos, data_neg, batch_size, loop, vocab_dict, shuffle=False):
    ...
    while not stop:  
        ...
        target_pos = [1] * (len(tensor_pad_l) // 2)
        
        # Generate the list of targets for the negative examples (a list of zeros)
        # The length is the number of negative examples in the batch
        target_neg = [0] * (len(tensor_pad_l) // 2)
        ...
        example_weights = np.ones(len(tensor_pad_l), dtype = 'int')

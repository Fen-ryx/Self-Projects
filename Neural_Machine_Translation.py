...
def average_overlap(similarity_fn, samples, *ignore_params):
    ...
    for index_candidate, candidate in enumerate(samples):    
        #print(index_candidate, "H", candidate)
        ### START CODE HERE
        #print(samples[index_candidate])
        # initialize overlap
        overlap = 0
...
def mbr_decode(sentence, n_samples, score_fn, similarity_fn, NMTAttn=None, temperature=0.6, vocab_file=None, vocab_dir=None, generate_samples=generate_samples, sampling_decode=sampling_decode, next_symbol=next_symbol, tokenize=tokenize, detokenize=detokenize):
    ...
    ### START CODE HERE ###
    # generate samples
    samples, log_probs = generate_samples(sentence, n_samples, NMTAttn, temperature, vocab_file, vocab_dir, sampling_decode, next_symbol, tokenize, detokenize)
